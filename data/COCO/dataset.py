...
from vis import vis_3d_pose, vis_2d_pose, draw_nodes_nodes, draw_nodes_nodes_nodes
import joblib
...
class MSCOCO(torch.utils.data.Dataset):
    def __init__(self, data_split, args):
        ...
        self.img_path = osp.join(cfg.data_dir, dataset_name, 'COCO')
        ...
        self.poses, self.shapes, self.cam_param_ss, self.cam_param_ts = self.load_data()
    ...
    def load_data(self):
        ...
        db = COCO(osp.join('data/COCO', 'COCO', 'annotations', 'person_keypoints_' + self.data_split + '2014.json'))
        ...
        if self.data_split == 'train':
            ...
            poses, shapes, cam_param_ss, cam_param_ts = np.array(poses), np.array(shapes), np.array(cam_param_ss), np.array(cam_param_ts)
            
            return img_paths, img_hws, joint_imgs, joint_valids, poses, shapes, cam_param_ss, cam_param_ts
    ...
    def get_joints_from_mesh(self, mesh, joint_set_name, cam_param):
        ...
        if joint_set_name == 'human36':
            ...
            joint_coord_cam = np.dot(self.joint_regressor_smpl, mesh)
        # projection
    ...
    def __getitem__(self, idx):
        ...
        joint_cam_smpl, joint_img_smpl = self.get_joints_from_mesh(mesh_cam, 'smpl', cam_param)
        pose_param = smpl_param['pose']
        shape_param = smpl_param['shape']
        pose_param = pose_param.reshape(1, -1).repeat(self.seqlen, axis=0)
        shape_param = shape_param.reshape(1, -1).repeat(self.seqlen, axis=0)
        ...
        joint_cam_smpl = joint_cam_smpl - joint_cam_smpl[self.smpl_root_joint_idx]
        ...
        if cfg.MODEL.name == 'pose2mesh_net':
            joint_img_ori = joint_img_h36m.copy()
            if flip:
            ...
            joint_img_ori = joint_img_ori.reshape(1, len(joint_img_ori), 2).repeat(self.seqlen, axis=0)
        # make new bbox
        ...
        joint_cam_smpl = j3d_processing(joint_cam_smpl, rot, flip, self.smpl_flip_pairs)
        ...
        if cfg.MODEL.name == 'pose2mesh_net':
            ...
            lift_joint_valid = np.ones((1, len(joint_cam[0]), 1), dtype=np.float32)
            ...
            if error > self.fitting_thr:
                reg_joint_valid[:], lift_joint_valid[:] = 0, 0
            ...
            reg_joint_valid = reg_joint_valid.repeat(self.seqlen, axis=0)
            lift_joint_valid = lift_joint_valid.repeat(self.seqlen, axis=0)

            inputs = {'pose2d': joint_img}
            targets = {'lift_pose3d': joint_cam, 'reg_pose3d': joint_cam_h36m, 'joint_img_ori': joint_img_ori, 'pose': pose_param, 'shape': shape_param, 'gt_joints24': joint_cam_smpl}
            meta = {'lift_pose3d_valid': lift_joint_valid, 'reg_pose3d_valid': reg_joint_valid}

            return inputs, targets, meta

        elif cfg.MODEL.name == 'posenet':
            # default valid
            joint_valid = np.ones((1, len(joint_cam[0]), 1), dtype=np.float32)
            joint_valid = joint_valid.repeat(self.seqlen, axis=0)
            # compute fitting error
            error = self.get_fitting_error(tight_bbox, self.joint_imgs[idx], joint_img_coco[:17], self.joint_valids[idx])
            if error > self.fitting_thr:
                joint_valid[:, :] = 0

            return joint_img, joint_cam, joint_valid, joint_cam_smpl
